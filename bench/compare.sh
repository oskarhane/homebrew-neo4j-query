...
run_timed() {
  ...
  "$NQ" "${NEO4J_ARGS[@]}" --format "$format" "$cypher" > "$outfile" 2>/dev/null
...
for entry in "${QUERIES[@]}"; do
  ...
  "$NQ" "${NEO4J_ARGS[@]}" --format json "$cypher" > "$json_file" 2>/dev/null
  "$NQ" "${NEO4J_ARGS[@]}" --format toon "$cypher" > "$toon_file" 2>/dev/null
